...
export default function ReportsPage() {
  ...
  const calculateAndSetStats = (invoiceList) => {
    ...
    const totalAmount = invoiceList.reduce((sum, inv) => sum + safeNumber(inv.grandTotal), 0);
    ...
    const totalTax = invoiceList.reduce((sum, inv) => {
      return sum + safeNumber(inv.sgstAmount) + safeNumber(inv.cgstAmount) + safeNumber(inv.igstAmount);
    }, 0);

    setStats({ totalInvoices, totalAmount, paidAmount, pendingAmount, totalTax });
  };
  ...
  const calculateAndSetMonthlySummary = (invoiceList) => {
    ...
    invoiceList.forEach(invoice => {
      ...
      summary[monthYear].tax += safeNumber(invoice.sgstAmount) + safeNumber(invoice.cgstAmount) + safeNumber(invoice.igstAmount);
    });
  ...
  if (!stats) return <div className="loading">Calculating reports...</div>;
